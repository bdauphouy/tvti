	...
	>
		...
		<ul
			class="{hasScrolled ? 'bg-white/70' : 'bg-primary/70'} {!menuIsOpen
				? 'hidden xl:flex'
				: ''} absolute left-0 top-full z-10 flex h-96 w-screen flex-col place-items-center items-center justify-center gap-16 text-2xl backdrop-blur-md transition-colors duration-300 xl:relative xl:top-0 xl:h-auto xl:w-auto xl:flex-row xl:items-center xl:bg-transparent xl:text-lg xl:backdrop-blur-0"
		>
			<li>
				<a
